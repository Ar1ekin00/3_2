...
host srv {
hardware ethernet (Вместо этого текста со скобками, вставляешь MAC-адрес srv, найти его можно командой снизу);
...
*Записать:*
```bash
sshuser ALL=(root:ALL) NOPASSWD: /usr/sbin/shutdown, /usr/sbin/reboot, /usr/bin/htop
# Копируй пути выше, если они у тебя те же на тех же машинах, что и у меня
```
...
**Команды:** Заходишь за sshuser на всех машинах и вводишь команды
```bash
su - sshuser
...
```bash
cp /var/lib/samba/private/krb5.conf /etc/krb5.conf # здесь спросят, хочешь ли ты перезаписать, нажимаешь Y
```
```bash
...
Возвращаешься в терминал без графической оболочки и вставляешь:
```bash
echo P@ssw0rd | kinit Administrator@ILOVE.SA
echo P@ssw0rd | kinit variant1_1@ILOVE.SA
echo P@ssw0rd | kinit variant1_2@ILOVE.SA
echo P@ssw0rd | kinit variant1_3@ILOVE.SA
reboot
```
...
Чтобы через графическую оболочку зайти в /share_1 заходишь в папку пользователя на рабочем столе, в поиске вводишь /share_1 и всё
Если не будут создавать файлы в /share_1 попробуй на server зайти в папку /mnt/raid_1 выйти из неё и попробовать снова поменять права. Проверка смены прав ls -l /mnt/raid_1
