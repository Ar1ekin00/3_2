...
### Для cli:
(Для отчёта) -> зайти на cli через графическую оболочку, открыть любую папку в строке поиска написать "smb://srv/" откроется папка, в которой можно создать любой файл. Можете создать файл на cli, далее открыть терминал srv и ввести "smbclient //localhost/" -> пароль P@ssw0rd -> командой ls вывести содержимое каталога, в том числе, созданный файл. Или создать файл на srv, и он должен появится на cli
