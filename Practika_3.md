...
mkdir -p /srv/testapp/site_latest
cd /srv/testapp/site latest
[root@srv site_latest]# ls
Dockerfile  html
[root@srv site_latest]# cat html/
[root@srv html]# cat index.html 
<h1>TestApp</h1><p>Debug page</p>
ФАЙЛ:
nano Dockerfile
СОДЕРЖИМОЕ:
[root@srv site_latest]# cat Dockerfile 
# site_latest/Dockerfile
FROM registry.altlinux.org/alt/alt:latest

# обновим пакетный кэш и установим httpd2
RUN set -eux; \
    apt-get update; \
    apt-get install -y \
        httpd2 \
...
RUN printf '<!DOCTYPE html><html><head><title>TestApp</title></head><body><h1>TestApp running on %s</h1><p>Connected to DB: testdb (user: test)</p></body></html>\n' "$(hostname)" > /var/www/html/index.html

EXPOSE 80

CMD ["httpd2", "-D", "FOREGROUND"]


КОМАНДЫ:
mkdir -p /srv/testapp/mariadb_latest
cd /srv/testapp/mariadb_latest
ФАЙЛ:
nano Dockerfile
СОДЕРЖИМОЕ:
[root@srv mariadb_latest]# cat Dockerfile 
# mariadb_latest/Dockerfile
FROM ubuntu:noble
...
# настройка datadir и пользователя
RUN mkdir -p /var/lib/mysql /run/mysqld && \
    chown -R mysql:mysql /var/lib/mysql /run/mysqld && \
    chmod 1777 /run/mysqld

# даём mariadbd запускаться от mysql
USER mysql
...
КОМАНДЫ:
nano /srv/testapp/mariadb_latest/healthcheck.sh
...
nano /srv/testapp/mariadb_latest/docker-entrypoint.sh

#!/bin/bash
set -e
mysql -h 127.0.0.1 -u root -p"$MYSQL_ROOT_PASSWORD" -e "SELECT 1"

[root@srv mariadb_latest]# cat docker-entrypoint.sh 
#!/bin/bash
set -euo pipefail

# используем gosu для запуска mariadbd от пользователя mysql
if [[ "$1" == "mariadbd"* ]]; then
    # если это первый запуск и нет mysql.*
    ...
    # меняем пользователя на mysql и запускаем mariadbd
    exec gosu mysql "$@"
fi

# если пришла любая другая команда — просто её выполняем
exec "$@"
...
services:
  ...
  testapp:
    ...
    volumes:
      - ./site_latest/html:/var/www/html:ro
    environment:
      DB_HOST: db
      DB_NAME: testdb
      DB_USER: test
...
КОМАНДЫ:
docker build -t mariadb_latest:latest .
docker build -t site_latest:latest .
docker compose up -d
